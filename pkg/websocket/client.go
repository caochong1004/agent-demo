...
import (
	...
	pipeutil "hci-agent/pkg/util/pipe"
	util_url "hci-agent/pkg/util/url"
	"net/http"
	"net/url"
	"sync"
	"time"
...
func (c *appClient) connect() error {
	...
	glog.V(0).Info("Connect to DevOps service success")
...
func (c *appClient) pipeConnection(id string, key string, token string, pipe pipeutil.Pipe, ) (bool, error) {
	...
	newURLStr := fmt.Sprintf(BaseUrl, newURL.Scheme, newURL.Host, key, key, c.clusterId, pipe.PipeType(), token)
	headers := http.Header{}
